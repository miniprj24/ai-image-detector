  ...
  return (
    <div className="bg-slate-800/40 backdrop-blur-md border border-white/5 shadow-glass rounded-2xl overflow-hidden mb-8 transition-all duration-300 hover:bg-slate-800/50 hover:shadow-glass-sm group">
      ...
      <div className="px-6 pt-2 space-y-2">
        ...
        {post.results &&
          Object.keys(post.results).map((model) => {

            const label =
              post.results[model] === "AI-generated"
                ? "AI Generated"
                : post.results[model] === "Human-made"
                ? "Human Made"
                : post.results[model];
            ...
            return (
              <div
                key={model}
                className={`text-xs font-bold px-4 py-2 rounded-full text-white w-fit ${badgeClass}`}
              >
                {model.toUpperCase()} - {label}
                {post.scores?.[model] && ` (${post.scores[model]})`}
              </div>
            );
          })}
          ...
          >
            <Heart
              className={`w-7 h-7 transition-all duration-300 ${
                liked
                  ? 'fill-red-500 text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.5)] scale-110'
